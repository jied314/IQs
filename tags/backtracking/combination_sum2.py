...
class CombinationSum2:
    ...
    def combination_sum2(self, candidates, target):
        ...
        candidates.sort()
        ret = []
        self.visit(candidates, 0, [], target, ret)
        return ret

    def visit(self, candidates, start, base, target, ret):
        if target == 0:
            ret.append(list(base))
        else:
            if target < 0:
                return
            for i in range(start, len(candidates)):
                if i > start and candidates[i] == candidates[i-1]:  # duplicate
                    continue
                base.append(candidates[i])
                self.visit(candidates, i+1, base, target-candidates[i], ret)
                base.pop()
