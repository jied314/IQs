...
class SortList:
    ...
    # 1/27 - borrow from Yanxing
    # Test on LC - 440ms, 30%
    # Nice easy merge sort on LL, use recursion
    def sort_list_merge(self, head):
        if head is None or head.next is None:
            return head
        mid = self.cur_middle(head)
        left = self.sort_list(head)
        right = self.sort_list(mid)
        return self.merge(left, right)

    # find the middle node and mid.next = None
    def cur_middle(self, head):
        fast, slow = head, head
    ...
    # iteratively merge to avoid maximum recursion
    def merge_iterative(self, l1, l2):
